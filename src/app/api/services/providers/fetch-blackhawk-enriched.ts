...
export interface EnhancedBlackhawkProduct extends BlackhawkProduct {
  cardExists: boolean;
  associatedItems: AssociatedItem[];
}

export interface BlackhawkApiResponse {
  data: {
...
const BLACKHAWK_API_URL =
  "https://us-west2-carepoyntops-09072017.cloudfunctions.net/blackhawk-proxy-prod";
...
/**
 * Fetches Blackhawk catalog data enriched with database information
 * @returns Promise<BlackhawkApiResponse> - The enriched catalog data in full API response format
 */
export async function fetchEnrichedBlackhawkCatalog(): Promise<BlackhawkApiResponse> {
  // Step 1: Fetch gift cards with their items and brands for provider_id = 4 (Blackhawk)
  const giftCards = await db.redemption_giftcards.findMany({
    where: {
      redemption_giftcard_items: {
        provider_id: 4, // Blackhawk provider ID
      },
      value: {
  ...
  // Step 1c: Fetch ALL gift cards for these items (not just Blackhawk ones)
  ...
  giftCards.forEach((giftCard) => {
    ...
    if (item?.utid) {
      ...
      const associatedItem: AssociatedItem = {
        ...
        source_letter: "C", // Blackhawk provider is always "C"
  ...
  // Step 4: Required payload for the Blackhawk API
  const payload = {
  ...
  // Step 5: Make POST request to Blackhawk API with required payload
  const response = await axios.post(BLACKHAWK_API_URL, payload, {
    headers: {
      "Content-Type": "application/json",
    },
  });

  // Step 6: Get the API response data
  const blackhawkData = response.data;

  // Step 7: Add cardExists field and associated items to each product in the response
  if (blackhawkData.data && blackhawkData.data.products) {
    blackhawkData.data.products = blackhawkData.data.products.map(
      (product: BlackhawkProduct): EnhancedBlackhawkProduct => {
        // Check if the contentProviderCode exists in our UTIDs set
        const cardExists = utids.has(product.contentProviderCode);

        // Get associated items for this product's contentProviderCode
        const associatedItems = associatedItemsMap.get(product.contentProviderCode) || [];
  ...
  // Step 8: Return the enhanced data in full API response format
  return blackhawkData;
}

/**
 * Fetches only the products array from Blackhawk catalog
 * @returns Promise<EnhancedBlackhawkProduct[]> - Just the enriched products array
 */
export async function fetchEnrichedBlackhawkProducts(): Promise<EnhancedBlackhawkProduct[]> {
  const response = await fetchEnrichedBlackhawkCatalog();
